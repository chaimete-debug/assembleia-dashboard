window.UI = (() => {
        ...
        <div class="p-4 md:p-6 bg-slate-50 border-b border-slate-200">
          ...
          <div class="flex flex-wrap gap-3 items-center">
            <div id="viewTabs" class="inline-flex flex-wrap gap-2 rounded-2xl bg-white border border-slate-200 p-2 shadow-sm">
              <button type="button" class="tab-btn bg-blue-700 text-white rounded-xl px-4 py-2.5 text-sm font-semibold border border-blue-700" data-view="all" aria-selected="true">Ver Tudo</button>
              <button type="button" class="tab-btn bg-white text-slate-700 rounded-xl px-4 py-2.5 text-sm font-semibold border border-slate-200" data-view="igreja" aria-selected="false">Igreja Local</button>
              <button type="button" class="tab-btn bg-white text-slate-700 rounded-xl px-4 py-2.5 text-sm font-semibold border border-slate-200" data-view="ministros" aria-selected="false">Pastores / Ministros</button>
              <button type="button" class="tab-btn bg-white text-slate-700 rounded-xl px-4 py-2.5 text-sm font-semibold border border-slate-200" data-view="sd" aria-selected="false">Superintendente</button>
              <button type="button" class="tab-btn bg-white text-slate-700 rounded-xl px-4 py-2.5 text-sm font-semibold border border-slate-200" data-view="juntas" aria-selected="false">Juntas</button>
              <button type="button" class="tab-btn bg-white text-slate-700 rounded-xl px-4 py-2.5 text-sm font-semibold border border-slate-200" data-view="day" aria-selected="false">Modo Dia</button>
            </div>

            <button type="button" id="resetDataBtn" class="bg-white hover:bg-red-50 text-red-700 rounded-2xl px-4 py-2.5 text-sm font-semibold border border-red-200">
              Limpar Dados
            </button>
          </div>
        </div>
  ...
  function phaseCard(phaseId, phase, tasksHtml) {
    ...
    return `
      <section class="phase-card rounded-3xl border ${c.card} overflow-hidden" data-phase="${phaseId}">
        <button class="phase-toggle w-full text-left p-5 md:p-6 flex items-start justify-between gap-4">
          <div>
            <div class="flex items-center gap-3 flex-wrap">
              <span class="${c.badge} text-xs font-bold px-3 py-1 rounded-full">${phase.code}</span>
              <span class="text-xs md:text-sm font-semibold ${c.subtitle}">${phase.subtitle}</span>
            </div>
          ...
          <span class="toggle-icon ${c.subtitle} text-sm font-bold mt-1">−</span>
        </button>
  ...
  function taskCard(task, phaseColor, saved) {
    ...
    return `
      <article class="task-card bg-white rounded-2xl border border-slate-200 p-4" data-task-id="${task.id}" data-groups="${task.groups.join(" ")}">
        <div class="text-sm font-bold ${c.titleTone}">${task.id} ${task.title}</div>
        <div class="text-sm text-slate-500 mt-1">Responsável: ${task.owner}</div>
        <div class="task-date-hint text-xs font-semibold ${c.range} mt-2"></div>
        ...
        <div class="mt-3 grid md:grid-cols-2 gap-2">
          ...
          <input class="date-input w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" type="date" value="${saved.date || ""}" />
        </div>

        <textarea class="notes-input w-full mt-2 rounded-xl border border-slate-300 px-3 py-2 text-sm" rows="2" placeholder="Observações...">${saved.notes || ""}</textarea>
      </article>
    `;
  }
